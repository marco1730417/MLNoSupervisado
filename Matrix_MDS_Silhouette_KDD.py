...
def open_archivo(nombre):
    ...
    for row in csv_ent: # Leer campos por separado (variables)
        lista.append(row)
    return lista 
abrir = open_archivo('C:\Users\Marco Ambuludi\Dropbox\Tesis Marco\DATASET\KDDprueba.csv');
y=[]
X=[]
...
np.savetxt("D:\TESIS\dataset\Datos\matrizdistancias.csv", D, delimiter=",")
###########################################################################
...
fig, ax = plt.subplots()
x=0 #que no genere problemas con el etiquetado normal y anormal en la imagen
ax.plot(x, label="Normal",color='b')
ax.plot(x, label="Anormal",color='r')
ax.legend();
plt.title('Escalamiento Multidimensional Dataset NSL-KDD')
MDS()
...
ax2.set_title("Objetos Agrupados NSL-KDD")
...
#D = D[idx1,:]
#D = D[:,idx1]
# a continuacion es el grafico Matriz de Distancias (azul) color= cmap
########################################
#############################################
...
im = axmatrix.matshow(D ,aspect='auto', cmap=pylab.cm.YlGnBu) # YlGnBu Escala de Azules; YlOrRd Escala de Rojos; YlGn Escala de Verdes
...
#axmatrix.yaxis.tick_left() Comentado por numeros en parte inferior y derecha
axmatrix.set_title('Matriz de Distancias del Dataset NSL-KDD')
